...
# ─── API: Scrape results for one job ─────────────────────────────────────────

@scraping_bp.route('/api/scrape_assessment_results', methods=['POST', 'OPTIONS'])
@rate_limit(max_calls=3, time_window=300)
def api_scrape_assessment_results():
    """Scrape RecruitAI assessment results for a specific job title"""
    if request.method == 'OPTIONS':
        return '', 200

    try:
        data          = request.json
        job_title     = data.get('assessment_name') or data.get('job_title')

        if not job_title:
            return jsonify({"success": False, "message": "assessment_name (job_title) is required"}), 400

        logger.info(f"[SCRAPER] Starting RecruitAI result scraping for: {job_title}")

        thread = threading.Thread(
            target=_run_with_monitoring,
            args=(job_title,),
            daemon=True,
            name=f"scraping_recruitai_{job_title.replace(' ', '_')}_{int(time.time())}"
        )
        thread.start()

        return jsonify({
            "success":        True,
            "message":        f"Started scraping RecruitAI results for '{job_title}'",
...
# ─── API: Scrape all completed assessments ────────────────────────────────────

@scraping_bp.route('/api/scrape_all_pending_results', methods=['POST', 'OPTIONS'])
@rate_limit(max_calls=1, time_window=600)
def api_scrape_all_pending_results():
    """Scrape RecruitAI results for all distinct job titles that have candidates"""
    if request.method == 'OPTIONS':
        return '', 200

    try:
        logger.info("[SCRAPER] Starting bulk RecruitAI result scraping")

        thread = threading.Thread(
            target=_run_bulk_with_monitoring,
            daemon=True,
            name=f"bulk_scraping_recruitai_{int(time.time())}"
        )
        thread.start()

        return jsonify({
            "success":        True,
            "message":        "Started bulk scraping for all job titles",
            "estimated_time": "5-15 minutes"
        }), 200
...
def api_scraping_status():
    ...
    try:
        active = [
            {"name": t.name, "is_alive": t.is_alive(), "daemon": t.daemon}
            for t in threading.enumerate()
            if t.name.startswith(('scraping_', 'bulk_scraping_'))
        ]
        return jsonify({
            "success":           True,
...
# ─── API: Get stored results ──────────────────────────────────────────────────

@scraping_bp.route('/api/assessment_results', methods=['GET', 'OPTIONS'])
def get_all_assessment_results():
    """Return candidates whose exam has been completed (scraped from RecruitAI)"""
    if request.method == 'OPTIONS':
        return '', 200

    session = SessionLocal()
    try:
        job_title = request.args.get('assessment_name') or request.args.get('job_title')
        min_score = request.args.get('min_score', type=float)
        max_score = request.args.get('max_score', type=float)

        query = session.query(Candidate).filter(Candidate.exam_completed == True)

        if job_title:
            query = query.filter(Candidate.job_title.ilike(f"%{job_title}%"))
        if min_score is not None:
            query = query.filter(Candidate.exam_percentage >= min_score)
        if max_score is not None:
            query = query.filter(Candidate.exam_percentage <= max_score)

        candidates = query.order_by(Candidate.exam_completed_date.desc()).all()
        ...
        for c in candidates:
            results.append({
                ...
                "score":            c.exam_percentage,
                "status":           "completed" if c.exam_completed else "pending",
                "final_status":     c.final_status,
                "completed_date":   c.exam_completed_date.isoformat() if c.exam_completed_date else None,
                "provider":         "recruitai",
            })
...
def get_assessment_statistics():
    """Assessment statistics from candidates table"""
    if request.method == 'OPTIONS':
        return '', 200
    ...
    try:
        ...
        if not candidates:
            return jsonify({"success": True, "statistics": {
                "total_candidates": 0, "average_score": 0,
                "min_score": 0, "max_score": 0, "pass_rate": 0
            }}), 200

        scores = [c.exam_percentage for c in candidates if c.exam_percentage is not None]
        ...
        statistics = {
            ...
            "status_distribution": {
                "passed":  len([c for c in candidates if c.final_status == "Assessment Passed"]),
                "failed":  len([c for c in candidates if c.final_status == "Assessment Failed"]),
                "pending": len([c for c in candidates if not c.exam_completed]),
            }
        }
...
# ─── INTERNAL: Run scraper with monitoring ────────────────────────────────────

def _run_with_monitoring(job_title: str):
    start = time.time()
    try:
        from app.services.assessment_result import run_result_scraper
        results = run_result_scraper(job_title)
        duration = time.time() - start
        count = len(results) if results else 0
        logger.info(f"[SCRAPER] Completed '{job_title}' in {duration:.1f}s — {count} result(s)")
        notify_admin(
            "RecruitAI Scraping Completed",
            f"Job: {job_title}\nResults: {count}\nDuration: {duration:.1f}s"
        )
    except Exception as e:
        duration = time.time() - start
        logger.error(f"[SCRAPER] Failed '{job_title}' after {duration:.1f}s: {e}", exc_info=True)
        notify_admin(
            "RecruitAI Scraping Failed",
            f"Job: {job_title}\nDuration: {duration:.1f}s",
            error_details=traceback.format_exc()
        )


def _run_bulk_with_monitoring():
    start = time.time()
    try:
        from app.services.assessment_result import run_result_scraper

        # Get all distinct job titles that have candidates
        session = SessionLocal()
        try:
            from sqlalchemy import distinct
            job_titles = [
                row[0] for row in
                session.query(distinct(Candidate.job_title))
                .filter(Candidate.job_title.isnot(None))
                .all()
            ]
        finally:
            session.close()

        logger.info(f"[BULK SCRAPER] Found {len(job_titles)} job title(s): {job_titles}")

        summary = {}
        for jt in job_titles:
            try:
                results = run_result_scraper(jt)
                summary[jt] = len(results) if results else 0
            except Exception as e:
                logger.error(f"[BULK SCRAPER] Failed for '{jt}': {e}")
                summary[jt] = 0

        duration = time.time() - start
        total    = sum(summary.values())
        logger.info(f"[BULK SCRAPER] Done in {duration:.1f}s — {total} total result(s)")
        notify_admin(
            "Bulk RecruitAI Scraping Completed",
            f"Jobs: {len(summary)}\nTotal results: {total}\nDuration: {duration:.1f}s\n\n" +
        ...
        duration = time.time() - start
        logger.error(f"[BULK SCRAPER] Failed after {duration:.1f}s: {e}", exc_info=True)
        notify_admin(
            "Bulk RecruitAI Scraping Failed",
